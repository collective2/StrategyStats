...
coalesce((select count(*) from c2ex_trades t where (t.systemid = r.systemid) and (Date(t.exittime) <= r.datestart) and t."result" > 0),0) as NumTradesWin,
coalesce((select count(*) from c2ex_signals s where (s.systemid = r.systemid) and (Date(s.tradedwhen) <= r.datestart) and s."action" in ('STC','BTC')),0) as NumExits,
coalesce((select avg(t."result") from c2ex_trades t where (t.systemid=r.systemid) and (Date(t.exittime) <= r.datestart) and (t."result" <= 0)),0) as AvgTradeLoss,  
coalesce((select avg(t."result") from c2ex_trades t where (t.systemid=r.systemid) and (Date(t.exittime) <= r.datestart) and (t."result" > 0)),0) as AvgTradeWin  
